<template>
  <div class="container-fluid">
	<div ref="waiver" class="waiver mx-auto">
		<div class="row col-xs-12 my-2 mx-0 px-0">
			<h3 ref="waiverTitle">{{appState.waiverVolunteer.title}}</h3>
		</div>
		<div class="row col-xs-12 my-2 mx-0 px-0">
			<p class="col-md-12 text-left mx-0 px-0 my-3">
				<span ref="waiverHeader0">{{appState.waiverVolunteer.header[0]}}</span>
                ...
                <span ref="waiverHeader1">{{appState.waiverVolunteer.header[1]}}</span>				<span ref="waiverHeader1">{{appState.waiverVolunteer.header[1]}}</span>
			</p>
		</div>
		<div class="row col-xs-12 my-2 mx-0 px-0">
			<p class="col-md-12 text-left mx-0 px-0 my-3">
				<span ref="waiverFooter">Please initial your agreement with each item below.</span>
			</p>
		</div>
		<div class="row col-xs-12 text-left my-3">
			<div v-for='index in appState.waiverVolunteer.items.length' 
				v-bind:key="index-1"
		...
		<div class="row col-xs-12 my-2 mx-0 px-0">
			<p class="col-md-12 text-left mx-0 px-0 my-3">
				<span ref="waiverFooter">{{appState.waiverVolunteer.footer}}</span>
			</p>
		</div>
		<div class="row col-md-12 my-5">
			<div class="col-md-8">
				<label class="form-label col-sm-12">Please type your full name*:</label>
				<input-row-no-label label="Signature" type="text"
					class="text-center mb-0"
			...
			<div class="col-md-4">
				...
				<input-row-no-label label="Today's Date" type="date"
					class="text-center"
...
export default {
	...
	data() {
		return {
			...
			isPrevious: true,
			isNext: false,
			prevRoute: "/application/volunteer/2",
			nextRoute: "",
			invalidFields: {}
		};
	},
	methods: {
		constructItemFields: function() {
			...
			return new Promise(
				(resolve, reject) => {
					for (let item in this.appState.waiverVolunteer.items) {
		...
		initTodayDate() {
			...
			this.appState.waiverVolunteer.signedDate = year + "-" + month_num + "-" + day;
		},
	},
	computed: {
		appState: {
		...
		getDateAsString() {
			...
			let dt = new Date(this.appState.waiverVolunteer.signedDate);
